...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if (empty($_POST['name']) || empty($_POST['price'])) {
        header("Location: /admin/create-product.php?" . http_build_query([
    ...
    $stmt = $db->prepare("INSERT INTO products (name, description, category_id, price, image) VALUES (:name, :description, :category_id, :price, :image)");
    $stmt->execute([
        'name' => $_POST['name'],
        'description' => $_POST['description'],
        'category_id' => $_POST['category_id'],
...
<div class="container">
    ...
    <form action="/admin/create-product.php" method="POST" class="content form"
            ...
            <select name="category_id" id="category_id">
                <?php foreach ($categories as $category): ?>
                    <option value="<?= $category['id'] ?>"
                        ...
                        <?= strval($category['id']) === ($_GET['category_id'] ?? '') ? 'selected' : '' ?>>
                        <?= htmlspecialchars($category['name']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
